...
def progress_bar():
    ...
    app.after(1500,lambda:progressbar_load2.destroy())
    ...
    def run_after():
        def let_gen():
            if((cpu.get(),ram.get(),storage.get(),part.get(),display.get(),software.get())==(0,0,0,0,0,0)):
                ...
                messagebox.showinfo("Message",f"It takes Few Seconds to Generate Report")
                generate(cpu.get(),ram.get(),storage.get(),part.get(),display.get(),software.get(),directory_path,software_gather)
...
title=tk.Label(app,text="HardwareCheck v1.0.0",font=("Impact",20),fg="White")
title.pack()
...
des=tk.Label(main_frame,text="A PYTHON GUI application is to Retrive Information about the Laptop Spec for Window Operating System  ",font=("impact",15))
