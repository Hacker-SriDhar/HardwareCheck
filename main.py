    ...
    def run_after():
        def let_gen():
            if((cpu.get(),ram.get(),storage.get(),part.get(),display.get(),software.get())==(0,0,0,0,0,0)):
                ...
                progressbar_load.set(10)
                generate(cpu.get(),ram.get(),storage.get(),part.get(),display.get(),software.get(),directory_path)
                file_path=directory_path+"/"+"report.pdf"
                if os.path.isfile(file_path):
                    progressbar_load.set(100)
                    messagebox.showinfo("Message",f"Report Generated")
                    app.after(2000,lambda:progressbar_load.destroy())
...
#,ram.get(),storage.get(),part.get(),display.get(),software.get()
...
app.title('System Rebuilt')
app.iconbitmap("Images/icon.ico")

title=tk.Label(app,text="HardwareCheck [^-^]",font=("Impact",20))
title.pack()
...
option_frame=tk.Frame(app,bg="#c3c3c3",highlightbackground="black",highlightthickness=2)
option_frame.pack(side=tk.LEFT)
option_frame.pack_propagate(False)
option_frame.configure(width=260,height=500)


main_frame=tk.Frame(app,highlightbackground="black",highlightthickness=2)
main_frame.pack(side=tk.LEFT)
main_frame.pack_propagate(False)
main_frame.configure(width=1600,height=500)


des=tk.Label(main_frame,text="The PYTHON GUI application is to Retrive Information about the Laptop Spec for Window Operating System     v1.0.0  ",font=("impact",15))
...
#background_thread=threading.Thread(target=get_software)  here
#background_thread.start()  here
